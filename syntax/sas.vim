...
hi def link sCard	Special
hi def link sDate_Time	Number
hi def link sKeyword	Identifier
"hi def link sFmtInfmt	Type
hi def link sString	String
"not used
"hi def link sText	term=NONE cterm=NONE ctermfg=White ctermbg=Black gui=bold guifg=Black guibg=White
hi def link sNumber	Number
hi def link sProc	Statement
hi def link sSection	Special
"hi def link mDefine	Function
hi def link mKeyword	PreProc
"hi def link mReference	term=NONE cterm=bold ctermfg=White ctermbg=Black gui=bold guifg=Blue guibg=White
"hi def link mSection	term=NONE cterm=NONE ctermfg=Blue ctermbg=Black gui=bold guifg=Navy guibg=White
"hi def link mText	Normal
